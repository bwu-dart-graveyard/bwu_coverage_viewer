...
Future<Map> collectCoverage(String script) async {
  ...
  Future connected =
      retry(() => VMService.connect('localhost', port), retryInterval);
  final vmService =
...
Future<Map> getAllCoverage(VMService service) {
  return service.getVM().then((vm) {
    return vm.isolates.map((i) => service.getCoverage(i.id));
  }).then(Future.wait).then((responses) {
...
const retryInterval = const Duration(milliseconds: 3000);
...
Future startProcess(String script, int port) async {
  ...
  final io.Process process = await io.Process.start(
      'dart', ['--observe=${port}', /*'--pause_isolates_on_start',*/ script]);
  process.exitCode.then((exitCode) => print('Script exit - code: ${exitCode}'));
